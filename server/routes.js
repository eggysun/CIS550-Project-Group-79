...
<<<<<<< HEAD
  ...
  // 1. We changed the user_id LIKE '${user_id}' to save allow for a unique index scan,
  //    which lowers the cost from 18.21 to 14.79
  // 2. We have replaced "FROM user u ... WHERE u.user_id = ..." to "(SELECT * FROM User WHERE user_id = '${user_id}') u".
  //    According to the concepts learned in class, we have pushed down the projections as much as possible.
  // 3. We have replaced cartisian products of the four tables to inner joins which save costs.

const friends_review = async function(req, res) {
  var user_id = req.params.user_id
  connection.query(`
    SELECT DISTINCT b.name AS name, b.business_id AS business_id, b.address AS address, b.city AS city, b.state AS state, b.postal_code AS postal_code, r.stars AS stars, f.friends AS friend_id
...
>>>>>>> b3e855aa08e4dd8e66f8f371954c4d2c47b83f07
    ...
    `, (err, data) => {
      if (err || data.length === 0) {
        console.log(err);
<<<<<<< HEAD
        res.json([{"name":"","business_id":"","address":"","city":"","state":"","postal_code":"","stars":"","friend_id":""}]);
=======
        res.json({});
>>>>>>> b3e855aa08e4dd8e66f8f371954c4d2c47b83f07
      } else {
...
<<<<<<< HEAD
// Optimization: using smaller relationship as as outer in a join
  ...
  if (!page) {
    ...
    WITH temp AS (
      SELECT r.business_id AS business_id, r.stars AS stars
        FROM Business b JOIN Reviews r ON r.business_id = b.business_id
        JOIN User u ON u.user_id = r.user_id
        WHERE u.user_id LIKE '%${user_id}%'
    )
    SELECT u.name AS user_name, u.user_id AS user_id, b.name AS business_name, b.business_id AS business_id, r.stars AS stars
    FROM temp t JOIN Reviews r ON r.business_id = t.business_id
    JOIN Business b ON r.business_id = b.business_id
    JOIN User u ON r.user_id = u.user_id
    WHERE r.stars <= t.stars + 1 AND r.stars >= t.stars - 1
    ORDER BY u.name
    `, (err, data) => {
      if (err || data.length === 0) {
        console.log(err);
        res.json([{"user_name":"","user_id":"","business_name":"","business_id":"", "stars":""}]);
      } else {
        res.json(data);
      }
    });
...
// Optimization: 
// 1. I used Join instead of LEFT OUTER JOIN to say cost
// 2. Used Smaller relation as the outer in a join
=======
...
<<<<<<< HEAD
  FROM Reviews b JOIN User a ON a.user_id = b.user_id
...
<<<<<<< HEAD
// Optimization: Used the smaller relation as the outer in a join
=======
>>>>>>> b3e855aa08e4dd8e66f8f371954c4d2c47b83f07
...
<<<<<<< HEAD
    ...
    FROM FriendsOf a JOIN FriendsOf f ON a.friends = f.user_id
    WHERE a.friends IN (
      SELECT friends
      FROM FriendsOf
